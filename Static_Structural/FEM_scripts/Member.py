# Class which represents a 3D finte element model
...
from FEM_scripts.BeamSegZ import Beam_seg_z
from FEM_scripts.BeamSegY import Beam_seg_y
import FEM_scripts.Fixed_and_Reactions
from FEM_scripts.LoadCombo import Load_combination
...
class Member3D():
    ...
    def __k_Unc(self):
        ...
        k = matrix([[A * E / L, 0, 0, 0, 0, 0, -A * E / L, 0, 0, 0, 0, 0],
                    ...
                    [0, 6 * E * Iz / L ** 2, 0, 0, 0, 2 * E * Iz / L, 0, -6 * E * Iz / L ** 2, 0, 0, 0,
                     4 * E * Iz / L]])
        ...
        for case, factor in combo.factors.items():
            ...
            for ptLoad in self.PtLoads:
                ...
                if ptLoad[3] == case:
                    ...
                    if ptLoad[0] == 'Fx':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_AxialPtLoad(factor * ptLoad[1], ptLoad[2], self.L()))
                    elif ptLoad[0] == 'Fy':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_PtLoad(factor * ptLoad[1], ptLoad[2], self.L(), 'Fy'))
                    elif ptLoad[0] == 'Fz':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_PtLoad(factor * ptLoad[1], ptLoad[2], self.L(), 'Fz'))
                    elif ptLoad[0] == 'Mx':
                        fer = add(fer, FEM_scripts.Fixed_and_Reactions.MAT_Torque(factor * ptLoad[1], ptLoad[2], self.L()))
                    elif ptLoad[0] == 'My':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_Moment(factor * ptLoad[1], ptLoad[2], self.L(), 'My'))
                    elif ptLoad[0] == 'Mz':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_Moment(factor * ptLoad[1], ptLoad[2], self.L(), 'Mz'))
            ...
            for distLoad in self.DistLoads:
                ...
                if distLoad[5] == case:
                    ...
                    if distLoad[0] == 'Fx':
                        fer = add(fer,
                                  FEM_scripts.Fixed_and_Reactions.MAT_AxialLinLoad(factor * distLoad[1], factor * distLoad[2],
                                                                            distLoad[3], distLoad[4], self.L()))
                    else:
                        fer = add(fer, FEM_scripts.Fixed_and_Reactions.MAT_LinLoad(factor * distLoad[1], factor * distLoad[2],
                                                                            distLoad[3], distLoad[4], self.L(),
                                                                            distLoad[0]))
    ...
    def PlotTorsion(self, combo_name='Combo 1'):
        ...
        Member3D.__plt.ylabel(
            'Torsional Moment (Warping Torsion Not Included)')  # Torsion results are for pure torsion. Torsional warping has not been considered
        Member3D.__plt.xlabel('Location')
        Member3D.__plt.title('Member ' + self.Name + '\n' + combo_name)
        Member3D.__plt.show()
                    ...
                    if case == distLoad[5]:
                        ...
                        if round(x1, 10) <= round(x, 10):
                            ...
                            if Direction == 'Fx':
                                ...
                                SegmentsZ[i].M1 -= (x1 - x2) * (
                                            2 * w1 * x1 - 3 * w1 * x + w1 * x2 + w2 * x1 - 3 * w2 * x + 2 * w2 * x2) / 6
